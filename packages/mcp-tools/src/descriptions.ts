...
const BASE: Record<string, string> = {
  ...
  recon_suggest_matches:
    'Run the deterministic matching engine over open records and on-chain settlements and persist its DRAFT suggestions (status "suggested"); each candidate is valued into the record currency — a same-currency stablecoin at face value, any other token priced at the settlement date with pinned price/FX snapshots, and an unpriceable settlement is left unmatched (open). Returns each suggested match with the record, the backing event, the applied amount, a confidence score, and a rule-by-rule rationale, plus counts of unmatched records and settlements. The engine scores; nothing is applied until a leg is confirmed via recon_confirm_match, and suggestions never affect exports until then.',
  recon_confirm_match:
    'Confirm one suggested match leg by id, transitioning it to "confirmed" and carrying through its pinned valuation (the stored fiat plus, for a volatile-token leg, the price/FX snapshot it was valued at); re-checks that the settlement is not over-applied and returns the parent record\'s freshly derived status (open/partially_matched/matched/overpaid). Only confirmed legs feed exports. Fails NOT_SUGGESTED if the leg was already actioned, MATCH_CONFLICT if confirming would over-apply the event.',
  recon_reject_match:
